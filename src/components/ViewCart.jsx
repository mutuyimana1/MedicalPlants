...
const ViewCart = () => {
  ...
  return (
    <div className="w-full">
      <SubHeader />
      <Modal title=<h1>Delete Cart</h1>open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
      <div>
      ...
      <div className="w-full lg:w-[80%] px-5 lg:px-0 lg:m-auto md:flex gap-5 ">
        <div className="w-full md:w-[70%] bg-white   mt-10 p-3">
          <div className="flex justify-between border border-gray-200 shadow-md px-3">
            <h1 className="p-5 ">Selcted Product ({cartItems.plants && cartItems.plants.length} items)</h1>
            <h1 className="text-lg font-medium p-5 ">
              Total: <span className="text-orange-500">{cartItems?.cartTotal} Price</span>
            </h1>
            ...
            <div className=" sm:flex items-center space-x-4 p-5">
                  ...
                  {is_plant_loading ? <Spin
                    indicator={<LoadingOutlined spin size={25} color="black" />}
                    className="text-black"
                  />:  <MdDelete color="orange" size={30} onClick={showDeleteConfirm}/>}
                  {/* </div> */}
                </div>
          </div>
          {cartItems.plants && cartItems.plants?.map((el) => {
            ...
            return (
              <div className="flex gap-5 border border-gray-200 shadow-md p-5">
               ...
               {el.plant.images.length>0 && <img
                ...
                />} 
                <p className="mt-5 sm:mt-10 w-[20rem]">
                 { el?.plant?.title}
                </p>
              </div>
            );
            ...
            {/* <Link href={"/shop"}> */}
              <div className="flex gap-2 mt-10 " onClick={()=>navigate("/shop")}>
